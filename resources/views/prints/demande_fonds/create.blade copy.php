...
<body>
     ...
     <header>
        <table border="1" cellspacing="0" width="100%">
            ...
            <tr>
                <td width="25%" style="vertical-align: middle; border-color: black; border: 0.4px; text-align: center; width: 25%; vertical-align: middle;" rowspan="2">
                    <img src="{{ public_path('images/logo2.png') }}" width="180">
                </td>
                <td width="50%" style="vertical-align: middle; border-color: black; border: 0.4px; text-align: center; width: 50%; vertical-align: middle; background-color: #82a0ba; font-size:12px;" colspan="2"><strong>ENREGISTREMENT</strong></td>
                <td width="25%" style="vertical-align: middle; border-color: black; border: 0.4px; text-align: center; width: 25%; vertical-align: middle; color: blue; font-size:12px;" rowspan="2"><strong>Réf : EN-AEE-11<br/>Version : 02<br/>Page 1/1</strong></td>
                </tr>
                <tr>
                <td width="50%" style="vertical-align: middle; border-color: black; border: 0.4px; text-align: center; width: 50%" colspan="2"><strong>DEMANDE DE FONDS</strong></td>
    ...
    <table width="100%">
        <tr>
            <td style="text-align: center">N° {{ $demande_fond->num_dem ?? '' }} du {{ date('d/m/Y',strtotime($demande_fond->created_at ?? '')) }}</td>
        </tr>
    </table>
    ...
    <table border="1" cellspacing="0" width="100%">
        <tr>
            <td width="60%" style="text-align: center">OBJET DE LA DEMANDE</td>
            <td width="20%" style="text-align: center">MONTANT</td>
            <td width="20%" style="text-align: center">OBSERVATION</td>
        </tr>
        <tr>
            <td style="padding: 10px">{{ strtoupper($intitule ?? '') }}</td>
            <td style="text-align: center; padding: 10px">{{ strrev(wordwrap(strrev($demande_fond->montant ?? ''), 3, ' ', true)) }} F CFA</td>
            <td  style="padding: 10px">{{ $demande_fond->observation ?? '' }}</td>
        </tr>
        <tr>
            <td colspan="3">Arrêté la présente demande de fonds à la somme de (en lettres) : <span style="font-weight: bold">{{ $montant_en_lettre ?? '' }} FRANCS CFA</span></td>
        </tr>
        <tr>
            <td colspan="3">
                <table width="100%">
                    <tr>
                        <td>Avis de la DCG</td>
                        <?php $colspan = 1; ?>
                        @if(isset($signataire_first))

                            @if ($signataire_first != null)
                                <td>&nbsp;</td>
                                <td style="text-align: center">{{ $signataire_first->libelle_fonction }}</td>

                                <?php $colspan = $colspan + 2; ?>
                            @endif
                            
                        @endif
                        @if(isset($signataire_last))

                            @if ($signataire_last != null)
                                <td>&nbsp;</td>
                                <td style="text-align: center">{{ $signataire_last->libelle_fonction }}</td>
                                <?php $colspan = $colspan + 2; ?>
                            @endif
                    ...
                    <tr>
                        <td colspan="{{ $colspan ?? 1 }}" style="height: 100px;"></td>
                    </tr>
                    <tr>
                        <td>Décision de la Direction Générale <br/><span style="font-style: italic ; font-size:12px">(pour les montants supérieurs à 250 000 f CFA)</span></td>
                        
                        @if(isset($signataire_first))
                            @if ($signataire_first != null)
                                
                                <td>&nbsp;</td><td style="text-align: center">{{ $signataire_first->nom_prenoms }}</td>
                            @endif
                        @endif
                        ...
                        @if(isset($signataire_last))
                            @if ($signataire_last != null)
                                ...
                                <td>&nbsp;</td><td style="text-align: center">{{ $signataire_last->nom_prenoms }}</td>
                            @endif
                        @endif
    ...
    <table width="100%">
        <tr>
            <td style="font-weight: bold;">N.B. : La facture définitive et autres justificatifs doivent être déposés à la DFC ou les structures comptables dans les 72 heures qui suivent la réception. </td>
        </tr>
    </table>
                ...
                <td>
                    ...
                    <table>
                        <tr>
                            ...
                            <td>
                                <table width="100%" style="margin-top: 20px; @if(count($signataires) > 1) text-align:center @else text-align:right @endif ">
                                    <tr>
                                        <td>
                                ...
                                <table width="100%" style="margin-top: 80px; @if(count($signataires) > 1) text-align:center @else text-align:right @endif ">
                                    <tr>
                                        <td>
                                            {{ $signataire->nom_prenoms }}
                                        </td>
                                    </tr>
