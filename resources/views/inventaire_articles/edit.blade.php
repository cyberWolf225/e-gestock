...
<div class="container">
    ...
    <datalist id="list_article">
        @foreach($datalist_magasin_stocks as $datalist_magasin_stock)
            ...
            ?>
            @if(!isset($qte_phys))
                <option value="{{ $datalist_magasin_stock->ref_articles }}->{{ $datalist_magasin_stock->design_article }}->{{ $datalist_magasin_stock->design_magasin }}->{{ $qte_theo ?? '' }}->{{ $qte_phys ?? '' }}->{{ $ecart ?? '' }}->{{ $datalist_magasin_stock->id }}->{{ $justificatif ?? '' }}->{{ $flag_valide ?? '' }}->{{ $id ?? '' }}->{{ $flag_integre ?? '' }}">{{ $datalist_magasin_stock->design_article }}</option>
            @endif
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-5">
                                <div class="form-group pr-0">
                                    ...
                                    <input placeholder="Sélectionnez la famille" title="" required list="design_fam" value="{{ $familless->design_fam ?? ''  }}" autocomplete="off" type="text" name="design_fam" class="form-control">
                                </div>
                            </div>
                            <div class="col-md-1">
                                ...
                                <table class="table table-bordered table-striped" id="myTable" width="100%">
                                    <thead>
                                        <tr style="background-color: #e9ecef">
                                            <th style="width:12%; text-align:center">Réf<span style="color: red"><sup> *</sup></span></th>
                                            <th style="width:36%; text-align:center">Article</th>
                                            <th style="width:10%; text-align:center">Qté Théo</th>
                                            <th style="width:10%; text-align:center">Qté Phys<span style="color: red"><sup> *</sup></span></th>
                                            <th style="width:10%; text-align:center">Ecart</th>
                                            <th style="width:20%; text-align:center">Justificatif</span></th>
                                            ...
                                            <th style="text-align:center; width:1%">
                                                ...
                                                {{-- <a onclick="myCreateFunction()" href="#" class="addRow"><svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-plus" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <tbody>
                                        ...
                                        @foreach($magasin_stocks as $magasin_stock) 
                                            ...
                                            ->where('magasin_stocks_id',$magasin_stock->id)
                                            ->whereDate(DB::raw('DATE(created_at)'),'<=', $fin_per)
                                            ->select([DB::raw("SUM(qte) as total_qte")])
                                            ->groupBy('magasin_stocks_id')
                                            ->get();
                                            foreach ($mouvements as $mouvement) {
                                                $qte_theo = $mouvement->total_qte;
                                                //$concerne = 1;
                                            }
                                            ...
                                            if ($inventaire_article!=null) {
                                                ...
                                                $qte_theo = $inventaire_article->qte_theo;
                                                $qte_phys = $inventaire_article->qte_phys;
                                                $ecart = $inventaire_article->ecart;
                                                $justificatif = $inventaire_article->justificatif;
                                                $flag_valide = $inventaire_article->flag_valide;
                                                $flag_integre = $inventaire_article->flag_integre;
                                            ...
                                            <tr>
                                                <td style="vertical-align: middle; text-align:center; border-collapse: collapse; padding: 0; margin: 0;">
                                                    <input title="Sélectionnez l'article" value="{{ $magasin_stock->ref_articles ?? '' }}" autocomplete="off" list="list_article" 
                                                    type="text" onkeyup="editDesign(this)" id="ref_articles[{{ $index }}]" name="ref_articles[{{ $index }}]" class="form-control ref_articles" 
                                                    style="display: none"
                                                    ...
                                                    >
                                                    <input title="" value="{{ $magasin_stock->id ?? '' }}" autocomplete="off" style="display: none" type="text" id="magasin_stocks_id[{{ $index }}]" name="magasin_stocks_id[{{ $index }}]" class="form-control magasin_stocks_id" 
                                                    ...
                                                    <input title="" value="{{ $id ?? '' }}" autocomplete="off" style="display: none" type="text" id="id[{{ $index }}]" name="id[{{ $index }}]" class="form-control id" 
                                                    ...
                                                    &nbsp;&nbsp;&nbsp;{{ $magasin_stock->ref_articles ?? '' }}
                                                </td>
                                                <td style="vertical-align: middle; border-collapse: collapse; padding: 0; margin: 0;">
                                                    <input title="Désignation de l'article sélectionné" value="{{ $magasin_stock->design_article ?? '' }}" autocomplete="off" onfocus="this.blur()" style=" padding-left:2px; background-color:transparent; border-color:transparent; display:none" type="text" onkeyup="editMontant(this)" id="design_article[{{ $index }}]" name="design_article[{{ $index }}]" class="form-control design_article" 
                                                    ...
                                                    >
                                                    &nbsp;&nbsp;&nbsp;{{ $magasin_stock->design_article ?? '' }}
                                                </td>
                                                <td style="vertical-align: middle; border-collapse: collapse; padding: 0; margin: 0;">
                                                    <input title="la quantité théorique de l'article en stock" value="{{ $qte_theo ?? '' }}" autocomplete="off" onfocus="this.blur()" style="background-color:transparent; border-color:transparent; text-align:center" type="text" onkeyup="editMontant(this)" name="qte_theo[{{ $index }}]" id="qte_theo[{{ $index }}]" class="form-control qte_theo" 
                                                ...
                                                <td style="vertical-align: middle; border-collapse: collapse; padding: 0; margin: 0;">
                                                    <input onkeypress="validate(event)" title="la quantité physique de l'article en stock" value="{{ $qte_phys ?? '' }}" autocomplete="off" type="text" onkeyup="editMontant(this)" name="qte_phys[{{ $index }}]" id="qte_phys[{{ $index }}]" class="form-control qte_phys" 
                                                ...
                                                <td style="vertical-align: middle; border-collapse: collapse; padding: 0; margin: 0;">
                                                    <input style="text-align:center; background-color:transparent; border-color:transparent" title="Ecart" value="{{ $ecart ?? '' }}" autocomplete="off" onfocus="this.blur()" type="text" name="ecart[{{ $index }}]" id="ecart[{{ $index }}]" class="form-control ecart" 
                                                ...
                                                <td style="vertical-align: middle; border-collapse: collapse; padding: 0; margin: 0;">
                                                    <input title="Justifier l'écart" value="{{ $justificatif ?? '' }}" autocomplete="off" type="text" onkeyup="editMontant(this)" name="justificatif[{{ $index }}]" id="justificatif[{{ $index }}]" class="form-control justificatif" 
                                                    ...
                                                    <input style="display: none" autocomplete="off" type="text" onkeyup="editMontant(this)" name="index[{{ $index }}]" class="form-control index" id="index[{{ $index }}]" value="{{ $index }}">

                                                </td>
                                                @if(isset($profil_responsable_stock))
                                                    <td style="vertical-align: middle; text-align:center; border-collapse: collapse; padding: 0; margin: 0;">
                                                        <input title="Valider l'inventaire de cet article" @if(isset($flag_valide))
                                                            @if($flag_valide === 1)
                                                                checked
                                                            @endif
                                                        @endif autocomplete="off" onfocus="this.blur()" style="background-color: #e9ecef" type="checkbox" name="flag_valide[{{ $index }}]" id="flag_valide[{{ $index }}]" 
                                                    ...
                                                    <td style="vertical-align: middle; text-align:center; border-collapse: collapse; padding: 0; margin: 0;">
                                                        <input title="Integrer l'inventaire de cet article" @if(isset($flag_integre))
                                                            @if($flag_integre === 1)
                                                                checked
                                                            @endif
                                                        @endif autocomplete="off" onfocus="this.blur()" style="background-color: #e9ecef" type="checkbox" name="flag_integre[{{ $index }}]" id="flag_integre[{{ $index }}]" 
                                                ...
                                                @endif
                                                <td style="vertical-align: middle; text-align:center; border-collapse: collapse; padding: 0; margin: 0;">
                                                    <a title="Retirer cet article de la liste de votre demande"  href="#" class="remove" 
                                        ...
                                        @foreach($magasin_stock2s as $magasin_stock2)
                                            ...
                                            $mouvements = DB::table('mouvements')
                                            ->where('magasin_stocks_id',$magasin_stock2->id)
                                            ->whereDate(DB::raw('DATE(created_at)'),'<=', $fin_per)
                                            ->select([DB::raw("SUM(qte) as total_qte")])
                                            ->groupBy('magasin_stocks_id')
                                            ->get();
                                            ...
                                            <tr>
                                                <td style="vertical-align: middle; text-align:center; border-collapse: collapse; padding: 0; margin: 0;">
                                                    <input title="Sélectionnez l'article" value="{{ $magasin_stock2->ref_articles ?? '' }}" autocomplete="off" list="list_article" 
                                                    type="text" onkeyup="editDesign(this)" id="ref_articles[{{ $index }}]" name="ref_articles[{{ $index }}]" class="form-control ref_articles" 
                                                    style="display: none"
                                                    @if(isset($flag_integre))
                                                        @if($flag_integre === 1)
                                                    ...
                                                    <input title="" value="{{ $magasin_stock2->id ?? '' }}" autocomplete="off" style="display: none" type="text" id="magasin_stocks_id[{{ $index }}]" name="magasin_stocks_id[{{ $index }}]" class="form-control magasin_stocks_id" 
                                                    ...
                                                    &nbsp;&nbsp;&nbsp;{{ $magasin_stock2->ref_articles ?? '' }}
                                                </td>
                                                <td style="vertical-align: middle; border-collapse: collapse; padding: 0; margin: 0;">
                                                    <input title="Désignation de l'article sélectionné" value="{{ $magasin_stock2->design_article ?? '' }}" autocomplete="off" onfocus="this.blur()" style=" padding-left:2px; background-color:transparent; border-color:transparent; display:none" type="text" onkeyup="editMontant(this)" id="design_article[{{ $index }}]" name="design_article[{{ $index }}]" class="form-control design_article" 
                                ...
                                <table width="100%">
                                    ...
                                    <tr>
                                        ...
                                        style="border: none;">
                                            ...
                                            <textarea placeholder="Écrivez votre commentaire ici" rows="1" id="commentaire" type="text" class="form-control @error('commentaire') is-invalid @enderror" name="commentaire" style="width: 100%;"></textarea>
                                        </td>

                                        <td colspan="2" style="border: none;">

                                            <button onclick="return confirm('Faut-il enregistrer ?')" title="Modifier l'inventaire" type="submit" name="submit" value="edit" class="btn btn-success">
                                                Modifier
                                            </button>
                                        </td>
                                    </tr>
...
<script>
    ...
    function myCreateFunction() {
        ...
        var nbre_rows = rows.length-2;
        var index = nbre_rows - 2;
        ...
        if(nbre_rows<nbre_ligne_inventaire){
            ...
            if (profil_responsable_stock === 1) {
                ...
                cell8.innerHTML = '<td> <input title="Valider l\'inventaire de cet article" autocomplete="off" onfocus="this.blur()" style="background-color: #e9ecef" type="checkbox" name="flag_integre['+index+']" class="flag_integre" id="flag_integre"> </td>';
