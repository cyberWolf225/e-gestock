...
<div class="container">
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">{{ __('Livrer un retour d\'article') }} 
                ...
                <div class="card-body">
                    ...
                    @endif
                    <form method="POST" action="{{ route('livraison_retours.store')  }}">
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                            ...
                            <table class="table table-bordered" id="myTable" style="width:100%">
                                <thead>
                                    <tr style="background-color: #e9ecef;">
                                        ...
                                        <th style="vertical-align:middle; text-align:center; width:12%">Qté Validée</th>
                                        <th style="vertical-align:middle; text-align:center; width:12%">Qté Servie<span style="color: red"><sup> *</sup></span></th>
                                        <th style="vertical-align:middle; text-align:center; width:15%">Coût</th>
                                        <th style="vertical-align:middle; text-align:center; width:25%">Observation</th>
                                ...
                                <tbody>
                                    ...
                                    <?php   $montant = $demande->qte_validee * $demande->cmup;
                                            ...
                                            $check='';
                                            $livraison_retours = DB::select("SELECT * FROM livraison_retours WHERE retours_id = '".$demande->id."' ");

                                            if (count($livraison_retours)>0) {
                                                foreach ($livraison_retours as $livraison_retour) {
                                                $statut = $livraison_retour->statut;
                                                $observation = $livraison_retour->observation;

                                                $montant = $livraison_retour->qte * $demande->cmup;
                                                $qte =  $livraison_retour->qte;
                                                }
                                    ...
                                    <tr>
                                        <td style="vertical-align:middle; text-align:center;">
                                            ...
                                            <input style="display: none" value="{{ $demande->id ?? '' }}" required type="text" id="retours_id" name="retours_id[]" class="form-control retours_id">
                                        </td>
                                        <td style="vertical-align:middle; text-align:left">
                                            {{ $demande->design_article ?? '' }}
                                        </td>
                                        <td style="vertical-align:middle; text-align:right">
                                            <input value="{{ strrev(wordwrap(strrev($demande->cmup ?? 0), 3, ' ', true)) }}" required onfocus="this.blur()" style="background-color: #e9ecef;text-align:center" type="text" onkeyup="editMontant(this)" name="cmup[]" class="form-control cmup">
                                        </td>
                                        <td style="vertical-align:middle; text-align:center">
                                            {{ strrev(wordwrap(strrev($demande->qte_validee ?? 0), 3, ' ', true)) }}
                                        </td>
                                        <td style="vertical-align:middle;">
                                            <input style="text-align:center" value="{{ strrev(wordwrap(strrev($qte ?? $demande->qte_validee ?? 0), 3, ' ', true)) }}" type="text" onkeyup="editMontant(this)" name="qte[]" class="form-control qte">
                                        </td>
                                        <td style="vertical-align:middle; text-align:right">
                                        ...
                                        <td style="vertical-align:middle;">
                                            <input value="{{ $observation ?? '' }}" type="text" name="observation[]" class="form-control observation">
                                        </td>
                                        <td style="vertical-align: middle; text-align:center">
                                            <input {{ $check }} type="checkbox" id="approvalcd_{{ $demande->id }}" name="approvalcd[{{ $demande->id }}]" class="approvalcd_{{ $demande->id }}" />
                                        </td>
                                    </tr>
                                    @endforeach
