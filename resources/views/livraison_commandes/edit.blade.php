        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header entete-table">
                    ...
                    @if($griser === 1)
                        {{ __(strtoupper('Livraison commande')) }} 
                    @else
                    
                        @if($profil_fournisseur!=null)
                            @if($libelle==='Retiré (Frs.)' or $libelle==='Livraison partielle' or $libelle==='Livraison partiellement validée' or $libelle === 'Livraison partiellement confirmée' or $libelle==='Livraison annulée (Comite Réception)' or $libelle==='Livraison partielle confirmée' or $libelle==='Livraison totale')
                                {{ __(strtoupper('Modification de la livraison de la commande')) }} 
                            @endif

                        @elseif($profil_comite_reception!=null)
                ...
                <div class="card-body">
                    ...
                    @endif
                    <form enctype="multipart/form-data" method="POST" action="{{ route('livraison_commandes.update') }}">
                        @csrf
                                    ...
                                    <table style="width:100%;"> 
                                        <tr>
                                            <td colspan="4" style="border-bottom: none">
                                                <span style="font-weight: bold">Dernier commentaire du dossier </span> : écrit par <span style="color: brown; margin-left:3px;"> {{ $nom_prenoms_commentaire_livraison ?? '' }} </span>&nbsp; (<span style="font-style: italic; color:grey"> {{ $profil_commentaire_livraison ?? '' }} </span>)
                                            ...
                                            <div class="row justify-content-center">
                                                ...
                                                <div class="col-md-3" style="margin-top:0px"> 
                                                    ...
                                                    @if($libelle==='Retiré (Frs.)' or $libelle==='Livraison partielle' or $libelle==='Livraison partiellement validée' or $libelle === 'Livraison partiellement confirmée' or $libelle==='Livraison annulée (Comite Réception)' or $libelle==='Livraison partielle confirmée' or $libelle==='Livraison totale')
                                                        ...
                                                        <button onclick="return confirm('Êtes-vous sûr d\'enregistrer cette livraison ?')" style="margin-left:10px;" type="submit" name="submit" value="livrer" class="btn btn-warning">Modifier la livraison</button>
                                    ...
                                    @if($type_profils_name === 'Comite Réception')
                                        ...
                                        @if($griser === null )
                                            <table style="width:100%; margin-bottom:10px;" id="tablePieceFrs">
                                                <tr>
                                                    <td style="width: 1px; white-space:nowrap">
                                                        Veuillez joindre procès verbal de réception (PV) : &nbsp;&nbsp;
                                                    </td>
                                                    <td>
                                                        <input type="file" name="pv_reception" class="form-control-file @error('pv_reception') is-invalid @enderror" id="pv_reception" accept="application/pdf">   
...
<script>
    ...
    editMontant = function(e){
        ...
        var qte_en_cours_de_livraison=tr.find('.qte_en_cours_de_livraison').val();
        qte_en_cours_de_livraison = qte_en_cours_de_livraison.trim();
        qte_en_cours_de_livraison = qte_en_cours_de_livraison.replace(' ','');
        qte_en_cours_de_livraison = Number(reverseFormatNumber(qte_en_cours_de_livraison,'fr'));
        ...
        var qte_theo = qte_en_cours_de_livraison + qte - qte_actuelle;

        if(qte_cotation < qte_theo){

            if(qte_cotation >= qte){

                var qte_autre_livraison = qte_en_cours_de_livraison - qte_actuelle;

                Swal.fire({
                title: '<strong>e-GESTOCK</strong>',
                icon: 'error',
                html: 'La quantité livrée ne peut être supérieure à la quantité commandée. Vous avez déjà une quantité de : ' + qte_autre_livraison + ' dans d\'autres livraisons.',
                focusConfirm: false,
                confirmButtonText:
                    'Compris'
                });
