            ...
            <div class="col-12">
                ...
                <div class="card-body bg-white">
                    ...
                    <tbody style="vertical-align: middle">                        
                        ...
                        @foreach($reponse_cotations as $reponse_cotation)
                            <?php 
                                $responseProcedureSet = $controller2->procedureSetReponseCotationFormat($reponse_cotation);
                                ...
                                $type_statut_reponse_cotations_libelle = null;
                            
                                $statut_reponse_cotation = $controller2->getLastStatutReponseCotation($reponse_cotation->id);
                                if($statut_reponse_cotation != null){
                                    $type_statut_reponse_cotations_libelle = $statut_reponse_cotation->libelle;
                                }
                                ...
                                $color = 'orange';
                                if($type_statut_reponse_cotations_libelle === "Cotation rejetée"){
                                    $color = 'red';
                                }

                                if($type_statut_reponse_cotations_libelle === "Cotation sélectionnée"){
                                    $color = 'green';
                                }
                                ...
                                $title_mieux_disant = "";
                                $href_mieux_disant = "";
                                $display_mieux_disant = "none";
                                $svg_mieux_disant = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-check2" viewBox="0 0 16 16">
                                ...
                                if($type_statut_demande_cotations_libelle === "Coté" or $type_statut_demande_cotations_libelle === "Cotation en cours d'analyse"){
                                    if($type_profils_name === 'Gestionnaire des achats' or $type_profils_name === 'Responsable des achats' or $type_profils_name === 'Responsable DMP'){
                                        ...
                                        $title_edit = "Analyser la cotation";
                                        $href_edit = "/mieux_disants/create/".$reponse_cotations_id_encryptString;
                                        $display_edit = "";
                                        
                                    }
                                }

                                if($type_statut_reponse_cotations_libelle === "Cotation en cours d'analyse"){
                                    if($type_profils_name === 'Gestionnaire des achats' or $type_profils_name === 'Responsable des achats' or $type_profils_name === 'Responsable DMP'){
                            
                                        $title_mieux_disant = "Modifier la sélection du mieux disant";
                                        $href_mieux_disant = "/mieux_disants/index/".$demande_cotations_id_encryptString;
                                        $display_mieux_disant = "";
                            ...
                            <tr>
                                ...
                                <td style="text-align: left; vertical-align:middle; font-weight:bold; width: 1px; white-space: nowrap; color: #7d7e8f">{{ $reponse_cotation->entnum ?? '' }}</td>
                                <td style="vertical-align:middle">{{ $reponse_cotation->denomination ?? '' }}</td>
                                ...
                                <td style="width: 1px; white-space: nowrap;vertical-align:middle; text-align:right;">
                                    @if(isset($reponse_cotation->created_at))
                                       {{ date("d/m/Y",strtotime($reponse_cotation->created_at)) }} 
                                    @endif
                                </td>
                                <td style="width:1px; white-space:nowrap;vertical-align:middle; font-weight:bold; color:{{ $color ?? 'black' }}">
                                    @if($type_statut_reponse_cotations_libelle != null)
                                       {{ $type_statut_reponse_cotations_libelle ?? '' }} 
                                    @endif
                                ...
                                <td style="width:1px; white-space:nowrap; vertical-align:middle; text-align:center">
                                    ...
                                    <a style="display:{{ $display_mieux_disant }};" title="{{ $title_mieux_disant }}" href="{{ $href_mieux_disant }}" class="btn btn-success btn-sm">
                                        <?php if(isset($svg_mieux_disant)){ echo $svg_mieux_disant; }  ?>
                                    </a>
